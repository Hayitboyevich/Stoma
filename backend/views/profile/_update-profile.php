...
<div class="doctor-modal-wrap">
    <div class="doctor-modal-fixed">
        ...
        <?php if(isset($model) && !empty($model->id)): ?>
            <input type="hidden" name="user_id" value="<?= $model->id; ?>" />
        <?php endif; ?>
        <div class="inline-form-wrap">
            <label>Имя</label>
            <input value="<?= isset($model) ? $model->firstname : ''; ?>" autocomplete="off" name="first_name" type="text" />
        </div>
        <div class="inline-form-wrap" style="float: right">
            <label>Фамилия</label>
            <input value="<?= isset($model) ? $model->lastname : ''; ?>" autocomplete="off" name="last_name" type="text" />
        </div>
        <br />
        <div class="inline-form-wrap">
            <label>Телефон</label>
        ...
        <div class="inline-form-wrap doctor-sections" style="float: right;">
            ...
            <select class="select2-instance" multiple name="categories[]">
                <option></option>
                <?php foreach(PriceList::find()->all() AS $item): ?>
                    <option value="<?= $item->id; ?>" <?= in_array($item->id,$selected_cats) ? 'selected' : ''; ?>><?= $item->section; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <br />
